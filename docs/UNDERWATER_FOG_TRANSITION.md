...
**2026-07-06:** попытка fix по §6 (pre-submerge `BelowSurfaceFogStrength = 1.0`, surface-map fallback, uniform `depthBelow`) **откачена** — регресс тёмных пятен на суше у луж и на дне; исходные симптомы не закрыты. См. **TD-FL-034** в [`TECH_DEBT_FLUIDS.md`](TECH_DEBT_FLUIDS.md).
