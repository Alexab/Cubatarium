...
## Актуальная точка исполнения — 2026-09-12

Код `3d61756f`, ветка `cursor_audit_impl`. [Сверка закрытия A01–A15 и детальный порядок Q0–Q10](E:/Work/Home/Cubatarium/docs/streaming/AUDIT_CLOSURE_REVIEW_2026-09-12.md) — источник текущего статуса и приоритетов; исторические разделы ниже сохранены для трассировки решений.

**G0–G4 не закрыты.** Q0–Q3 landed; Q4 partial, Q5–Q10 partial (см. [сверку 2026-09-12](E:/Work/Home/Cubatarium/docs/streaming/AUDIT_CLOSURE_REVIEW_2026-09-12.md#implementation-progress-2026-09-12)). Локальные регрессии проходят (18/18 CTest), но smoke3 остаётся CORRECTNESS_FAIL. Дополнительно воспроизведён PASS оценщика при отсутствующих raw метриках (они превращаются в 0), а push-filter CI не включает текущую ветку. Поэтому актуальный порядок: **Q0 строгая схема/verdict → Q1 обязательный CI → Q2 renderer/light oracle и Q3 атрибуция schedule cost → Q4 полные immutable inputs → Q5 memory-constrained publication → Q6 owner cutover → Q7 admission/capture → Q8 deadline/telemetry → Q9 world acceptance → Q10 boundaries/обоснованные оптимизации**. Зависимости и допускаемое перекрытие работ указаны в новой сверке.

Прежнее предложение сразу продолжать с schedule-policy bottleneck уточнено: его измерение допустимо, но приёмочный оценщик сначала должен перестать выдавать ложный PASS. 17.60 ms — стоимость всего prep_schedule_policy, не доказанный вклад одного вызова IsSpawnMeshRingReady. Полный откат ветки не требуется; сохраняем уже проверенные fixes и исправляем оставшиеся контракты по отдельности.
