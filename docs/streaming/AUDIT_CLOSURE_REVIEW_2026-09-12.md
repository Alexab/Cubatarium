...
| G1 | Progress PASS / **product NOT closed** | dual-Q + RemeshQ snapshot (P1) + kick quota (P2) + debt age (P3) + SmallOracle (P4). Progress baseline `210134`. North autofly P5 cold/warm `230002`/`230452` (yaw **90**, VB med **21**) is **not** 174657-class. West manual FAIL `080455`/`080523`: VB med **75**, `focus_missing≈0.94`. New west manual FAIL `100645` / enter `100713`: VB all/fly med **51/76**, `focus_missing` med **1**, `miss_stuck` max **616**, `gpu_kick` fly med **0** with StaleVL/FullyDark debt ~**74**. Product proxy `product-174657` + **RelightReplace Dirty sole-owner** (`c3010053`): F5 early west VB still ~90–109 and proxy miss/stuck remains **0**. `proxy_v2` now fail-closes explicitly on adequacy (`112755`, visible `113011`) instead of false-greening medians; post-drain kick telemetry landed but is not exercised until proxy/manual reaches focus-miss class. A10 **partial** (FullyDark secondary Dirty gated); E3 remains explicit **FAIL/freeze**, not CLOSED. World-path FalseNegCull is no longer hardwired to 0, but dedicated object-id FBO sampling is still open. |
...
Следующий шаг: manual eye 174657-class west на RelightReplaceOwner ON; world object-id FalseNegCull (E4). Floors запрещены.
