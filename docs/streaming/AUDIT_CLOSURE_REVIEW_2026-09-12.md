...
| A10 / M11 | **Частично.** RelightReplace Dirty sole-owner ON (`c3010053`): secondary FullyDark Dirty pumps gated; MarkRelit→RemeshQ sole. ColumnRecord Decide ladder already EvictionOwner. Product early-west VB still FAIL class on `product-174657` F5 | Full M11 debt lifetime / FirstMesh Dirty writers still legacy; G1 product CLOSED awaits manual eye |
...
| G1 | Progress PASS / **product NOT closed** | dual-Q + RemeshQ snapshot (P1) + kick quota (P2) + debt age (P3) + SmallOracle (P4). Progress baseline `210134`. North autofly P5 cold/warm `230002`/`230452` (yaw **90**, VB med **21**) is **not** 174657-class. West manual FAIL `080455`/`080523`: VB med **75**, `focus_missing≈0.94`. West manual FAIL `100645` / enter `100713`: VB all/fly med **51/76**, `focus_missing` med **1**, `miss_stuck` max **616**, `gpu_kick` fly med **0** with StaleVL/FullyDark debt ~**74**. West manual FAIL `122212` / enter `122244`: same corridor `(7,3)→(−3,3)`, VB all/fly med **78/78.5**, `focus_missing` med **1**, `miss_stuck` max **311**, StaleVL ~**77**, `gpu_kick` fly med **0**; post-drain kick fired once without clearing stuck. **P3 FM-first visual regress** west manual `134914` / enter `134941` vs `122212`: VB fly **84**, StaleVL **87**, `miss_stuck` **197/496**, `dirty_remesh` **79**, unlit max **39** (unfinished **0** better). Proxy cold `133817` vs pre-P3 `125123`: VB **94** vs **80.5**, StaleVL **94.5** vs **80**. Root: `ShouldDeferRemeshSnapshotForFocusMiss` order-toggle (A10/A11 anti-pattern) — `ok_fm≈0` while RemeshLit starved. **Dual-lane landed** (defer removed + `ComputeDualLaneSchedule`); autofly S1/S3 gates green vs `125123`/`133817` (see `dual_lane_s3_gates.json`); **west manual eye pending** vs `122212`. `proxy_v2` climb FAIL (`112755`/`113011`). **`proxy_v3` cold/warm adequacy PASS** (`125123`/`125331`) as harness only. A10 **partial**; E3 **FAIL/freeze**. Object-id FBO sampling still open. |
...
Следующий шаг: **west manual eye** vs `122212` (confirm dual-lane closed `134914` black-chunk regress); object-id FBO (E4); S5 FirstMesh Dirty sole-owner / `ok_fm≈0` consumer. Floors / SoftDefer-for-holes запрещены.
