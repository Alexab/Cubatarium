...
| G1 | Progress PASS / **product NOT closed** | dual-Q + RemeshQ snapshot (P1) + kick quota (P2) + debt age (P3) + SmallOracle (P4). Progress baseline `210134`. North autofly P5 cold/warm `230002`/`230452` (yaw **90**, VB med **21**) is **not** 174657-class. West manual FAIL `080455`/`080523`: VB med **75**, `focus_missing≈0.94`. West manual FAIL `100645` / enter `100713`: VB all/fly med **51/76**, `focus_missing` med **1**, `miss_stuck` max **616**, `gpu_kick` fly med **0** with StaleVL/FullyDark debt ~**74**. West manual FAIL `122212` / enter `122244`: VB all/fly med **78/78.5**, StaleVL ~**77**, `miss_stuck` max **311**. **P3 FM-first visual regress** `134914` / `134941` vs `122212`: VB **84**, StaleVL **87**, unlit max **39**. **Dual-lane landed**; autofly S1/S3 gates green. **West manual eye PASS `154921` / enter `154948`**: VB **77**, StaleVL **75**, dirty_remesh **50**, ok_remesh **2**, cruise unlit max **13** — closes `134914` black-chunk class; wall **~71 ms** (same class as `122212` ~78 ms, not dual-lane regress). Enter `live_blockers` debt **80**. Product vs 141350 still FAIL. `proxy_v2` climb FAIL; **`proxy_v3` adequacy harness PASS**. A10 **partial**; E3 visual dual-lane **PASS**, product **OPEN**. Object-id FBO still open. |
