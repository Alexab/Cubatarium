...
6. **LOD и дальняя видимость имеют отдельную структуру.** Geometry clipmaps используют вложенные viewer-centered grid-уровни и постепенно обновляют их при движении, чтобы поддерживать frame rate и контролируемо снижать детализацию. Это подтверждает, что дальний обзор не следует обслуживать только расширением полного-resolution chunk ring. [Losasso & Hoppe, Geometry Clipmaps](https://hhoppe.com/proj/geomclipmap/).
7. **Запрос стриминга может идти от видимого результата.** GigaVoxels связывает выбор детализации и производство данных с информацией, полученной из текущего rendering/ray traversal. Это полезный архитектурный вариант для будущего LOD/visibility streaming, но не основание переписывать текущий greedy chunk renderer до исправления его publish lifecycle. [Crassin et al., GigaVoxels](https://www-sop.inria.fr/reves/Basilic/2009/CNLE09/).
8. **Empty — только подтверждённый результат построения.** В GPU Gems 3 block помечается empty после проверки, что генерация не произвела polygon output; отдельный список исключает такие блоки из повторной генерации. Для текущего mesh cache аналогом должен быть version-stamped `VerifiedEmpty` outcome, с причиной (air/fully occluded/empty geometry) и source revisions, отличный от not-built, stale или hidden-by-gate. [NVIDIA GPU Gems 3, Generating Complex Procedural Terrains Using the GPU](https://developer.nvidia.com/gpugems/gpugems3/part-i-geometry/chapter-1-generating-complex-procedural-terrains-using-gpu).
...
## Исполнение: пользовательское уточнение «пустые чанки» и lane backpressure, 2026-09-25

- Пользователь уточнил, что в последнем визуальном пролёте чёрных чанков не было, но было много визуально пустых чанков; наличие самой структуры мира глазами определить нельзя. Считаем это отдельным симптомом отсутствующей/непоказанной геометрии. Black/stale-light census не заменяет проверку видимой геометрии.
- Коммит `f983886f` пересчитывает dual-lane split по текущей FirstMesh/remesh demand вместо переноса прежних lane weights. Контрольный Release-пролёт сохранил скорость `5.09994` блока/с, но не сошёлся: `unfinished_visual` около 80, к остановке `column_not_ready=92`, `holes_rate=1.0`; report `pass=false`.
- Runtime capture обнаружил следующий override: при заполненном LightRepair reserve внутренний scheduler возвращал remesh quota к 4 даже при внешнем split `3 FirstMesh / 1 remesh`. В кадрах с 68–89 `unfinished_visual` FirstMesh schedule был 0, а четыре слота забирали remesh. Коммит `222cd48e` ограничил LightRepair reserve квотой, полученной от dual-lane admission.
- После `222cd48e` контрольный Release no-teleport пролёт остался на скорости `5.09994` блока/с. Измерения подтвердили `3 FirstMesh + 1 remesh` под backpressure: `cruise_dirty_fm_med=51` (было 2), `dirty_med=100` (было 315), schedule median — 4; промежуточный output backlog всё ещё достигал 56 queued GPU apply и 57 async mesh jobs. Это улучшило именно service split, но не устранило симптом: `holes_rate=1.0`, `unfinished_visual=78`, `chunk_not_ready_med=78`, после остановки `post_stop_missing_max=95`, post-stop convergence — FAIL. Видимый-black максимум составил 56; это не соответствует пользовательскому наблюдению о пустой геометрии и сам по себе не является подтверждением pixel-level симптома.
- Уточнён смысл телеметрии: `ColumnLoadedNoMeshN` в `WorldStreaming.cpp` присваивается из `FocusNotRenderReady`, поэтому имя вводит в заблуждение — это количество колонок, не прошедших render-ready/face-debt критерий, а не прямой подсчёт непустых voxel-срезов без mesh. `CountUnfinishedVisualNear` тоже считает колонку, если terrain-cache сообщает complete, но `IsColumnRenderReady` или face-debt не готовы. Отдельный `HasMissingGreedyMeshInHorizontalRadius` проверяет непустые блоки в загруженных чанках и отсутствие drawable mesh (с редкими пробами блоков через шаг 4); его `focus_missing_mesh` наблюдался на пролёте, что доказывает наличие voxel-данных без drawable mesh хотя бы у части срезов. Ни один из этих счётчиков не идентифицирует каждый видимый пользователю пустой участок или точное состояние пикселей.
- К остановке FM queue почти опустела (`dirty_fm_n=0`), но render-not-ready колонок стало 95; оставались GPU/async/completed items. Следующий разбор должен разделить по точным `{column,slice,incarnation}` и reason-кодам: terrain/section отсутствует, solid voxel data есть без mesh, build in-flight, GPU queued/kicked, drawable mesh есть, но не попал в draw. Затем проверить, почему завершение FM очереди не повышает render-ready coverage. Сначала не расширять квоты: на середине пути уже были крупные CPU/GPU output очереди.
- Release target `Cubatarium` и проверка статической Windows EXE прошли; тесты не запускались. Артефакты: [FirstMesh-priority report](../../bin/suite_reports/engine_refactor/fm_priority_product174657_20260925.json), [bounded-LightRepair report](../../bin/suite_reports/engine_refactor/fm_repair_cap_product174657_20260925.json), [perf JSONL](../../bin/logs/perf_20260925-164726_32632.jsonl).

## Исполнение: отделение voxel occupancy от пустого изображения, 2026-09-25

- Пользователь уточнил, что визуально пустые чанки не позволяют понять, сохранилась ли воксельная структура. Колонковые `unfinished_visual` и `visible_black` отвечают на другие вопросы и не устанавливают ни occupancy, ни наличие draw submission на видимых пикселях.
- Коммит `09d3411c` добавил opt-in focus census под `CUBA_VISUAL_BLACK_TRACE`: число загруженных solid/air/absent вертикальных срезов, точное число non-air voxels по счётчику `UChunk`, а для вертикального render band — drawable mesh, ожидающую работу, состояние `IsChunkSliceRenderReady` и live GPU draw. Коммит `a2e77783` уточнил различие между отсутствием drawable и принятым mesh-результатом без геометрии.
- В видимом Release no-teleport `product-174657` на nominal скорости `5.09994` блока/с (10 чанков / 160 блоков) получено 14 валидных moving census samples. Медиана: 393 resident solid slices, 1,240,186 non-air voxels, 197 drawable slices, 184 solid slices без drawable mesh, но принятых как satisfying empty, 28 срезов без drawable с dirty/inflight work; медиана draw-gate-closed — 11. Максимум unresolved no-work — 19, медиана — 0. Это подтверждает, что данные мира есть, но не устанавливает, что конкретные accepted-empty или unresolved slices находились в пикселях пользовательских дыр.
- Анализ `HasMeshSatisfyingColumnReady` показывает, что solid slice без drawable считается готовым при `GpuResident && GpuQuadCount == 0` либо когда CPU batches пусты, если не активны SoftDefer/defer-until-lit. Это предусмотрено для fully occluded mesh. В самом readiness predicate нет независимого occlusion witness или проверки текущего геометрического source revision; отдельный dirty флаг также не отменяет ready-ответ для cached empty. Это проверяемая гипотеза ложного зелёного состояния, но её причинность ещё не подтверждена кадро-пиксельным сопоставлением.
- Общий прогон по-прежнему FAIL: `holes_rate=1.0`, `unfinished_visual` median `78`, `fly_visible_black_max=66`, после stop convergence не наступил. Скорость нормальная. Новые счётчики диагностические; код поведения рендера этим изменением не менялся.
- Следующий шаг: для конкретных resident solid slices фиксировать coordinate, incarnation, mesh publish/source revision, `Dirty`/inflight/pending state, zero-geometry acceptance reason и фактический renderer submit/cull verdict в одном sample. Затем отделить: (1) полностью occluded empty с проверенными source stamps; (2) устаревший empty result при новом geometry demand; (3) solid без владельца mesh job; (4) drawable mesh, закрытый draw gate/frustum/pixel path. Только после проверки stale-empty ownership менять readiness predicate или его demand semantics.
- Артефакты: [refined census report](../../bin/suite_reports/engine_refactor/readiness_voxel_mesh_census_product174657_20260925.json), [perf JSONL](../../bin/logs/perf_20260925-173144_30032.jsonl).

## Исполнение: воксельная структура есть, draw gate закрыт, 2026-09-25

- Пользователь уточнил, что «пустое изображение» не показывает, пусто ли само voxel storage. Occupancy нужно измерять отдельно от draw readiness и видимого pixel output. В коммите `5013e008` focus trace получил точный stale-light witness: мировую координату блока, световой sample, face, CPU/GPU probe path, source chunk incarnation и light revision. Коммит `14a5744d` заполнил также текущую light revision target chunk и light revision, запечённую в mesh; до этого focus-slice trace оставлял эти два поля нулевыми.
- Первый видимый Release no-teleport прогон с witness (`perf_20260925-183952_18324.jsonl`) шёл на измеренной скорости `5.09994` блока/с, прошёл 10 чанков / 160 блоков и завершился с `holes_rate=1.0`, `fly_visible_black_max=78`, convergence FAIL. В bounded focus trace было 726 state-3 наблюдений (drawable mesh, draw gate закрыт); все 726 были fully-dark, без lit face, и находили positive-light witness по CPU mesh path. В 689 наблюдениях световой sample находился в самом target chunk, в 37 — в соседнем; target mesh publish-light revision равнялась 0, demand желала revision 1. Этот run указывает на stale bake в основном внутри чанка, но его target field revision была не записана старой версией trace, поэтому этого run недостаточно для полноценного сравнения source и target stamps.
- Повторный видимый Release no-teleport прогон после `14a5744d` (`perf_20260925-184321_27844.jsonl`) всё ещё FAIL: `holes_rate=1.0`, `fly_visible_black_max=66`, post-stop convergence — FAIL. Median скорости на движущихся samples `5.09994` блока/с. В 585 state-3 наблюдениях по 16 уникальным координатам `non_air_blocks=4096` у каждого target chunk; все имели satisfying drawable mesh, live GPU draw, fully-dark mesh и закрытый draw gate. На всех 585 sample текущая field revision совпадала с baked mesh revision и demand desired-light revision; stale-light witness не найден. Одновременно все 585 имели pending GPU apply/extract, 103 — pending light, 48 — явное LightRepair, 581 — active attempt. 483 наблюдения оставались на `Admitted`; средний demand-progress age составил около 13 секунд, максимальный — 22.3 секунды. Это прямое подтверждение voxel occupancy для выбранных срезов, но не доказательство, что эти координаты покрывали именно пустые пиксели окна.
- Два соседних холодных/прогретых прохода показали разные состояния того же симптома: stale baked light на первом и полностью тёмный mesh с совпадающими revisions плюс outstanding GPU/repair work на втором. Не следует сводить их к одной причине по одному proxy. Следующий рендерный шаг — reason trace для полного dark draw gate с фазой GPU ticket (queued/kicked/dispatched), terminal/LegalDark proof, и отдельная выборка кадра/depth/object-id для тех же chunk coordinates. Не менять policy, которая публикует законно тёмную геометрию, пока не подтверждены пиксельная видимость и назначение очереди.
- Отдельно подтверждена проблема адекватности пролёта. Второй прогон покрыл только 6 чанков / 96 блоков и остался `west-route coverage=UNTESTED`, несмотря на нормальную медиану скорости. В одном периоде при накопившихся 28 spikes максимум кадра был `1293 ms`. `compute_product_174657_input_adequacy()` в `flight_sim_run.py` рассчитывает скорость только по строкам `movement_speed > 2`, поэтому reported median не включает остановки/блокировки. Камера применяет collision resolution через `World::ResolveMovement`; свободное движение ограничивает frame dt и число physics substeps. Сейчас flight acceptance не записывает фактическое смещение относительно forward intent или суммарное время заблокированного движения, поэтому нельзя приписать недолёт ни столкновению, ни hitch. Добавить эти метрики в отдельный контрольный шаг, сохраняя GUI/no-teleport маршрут.
- Артефакты: [stale-witness report](../../bin/suite_reports/engine_refactor/stale_witness_product174657_20260925.json), [stale-witness perf](../../bin/logs/perf_20260925-183952_18324.jsonl), [revision-complete report](../../bin/suite_reports/engine_refactor/stale_witness_revisions_product174657_20260925.json), [revision-complete perf](../../bin/logs/perf_20260925-184321_27844.jsonl).
