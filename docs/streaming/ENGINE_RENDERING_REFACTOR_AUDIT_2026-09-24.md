...
- Следующий шаг — не расширять приоритет FirstMesh без доказанного эффекта. Разделить snapshot `Deferred` причины, FirstMesh capture/admission, LightRepair ticket → relight apply → mesh enqueue, и publication readiness на одном воспроизводимом missing slice. Для collision нужно добавить измерение requested-vs-resolved camera displacement и провести короткий no-teleport contact flight с контролируемым obstacle.
