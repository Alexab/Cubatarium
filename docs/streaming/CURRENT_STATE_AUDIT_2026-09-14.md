...
| N04 / sticky mid black | **REOPEN→ticket remesh; T2 unfreeze** | autofly `202118`: mid stalled=0 (equal-rev → LegalDark); eye_proxy PASS; VB still high — G1 OPEN |
...
| S2b / T2 mid FullyDark | **unfreeze** — equal-rev LegalDark census; mid stalled=0 on `202118`; MarkRelit force after ticket drain |
...
| Autofly `202118` | **post-N01 P3 T2** | 5 (Δ=0) | **0** | 0 | equal-rev → LegalDark; mid_stalled_gate PASS; eye KEEP |
...
C1 KEEP; post-N01 P0–P3: thrash cut + T2 LegalDark census (mid stalled=0);
eye_proxy incomplete mid≤5. G1 remains **OPEN** (VB fly still above dual-lane).
