...
- **G1 manual west FAIL (080455 / enter 080523):** no-teleport west `(7,3)→(−3,3)`; VB med **75**, `focus_missing≈0.94`, `miss_stuck` max **573**; progress telem (`ok_remesh`/`schedule_ok`/`kick`) live. G1 product **NOT closed**. Diff matrix: `bin/suite_reports/g1_a10_relight/autofly_vs_manual_diff.md` (root: north yaw90 vs west yaw180).
- **G1 manual west FAIL (100645 / enter 100713):** same no-teleport west corridor `(7,3)→(−3,3)` after RelightReplace Dirty sole-owner. VB all/fly med **51/76**, `focus_missing` med **1**, `miss_stuck` max **616**, `gpu_kick` fly med **0** (non-zero only sporadically), oracle StaleVL/FullyDark debt fly ~**74**. E3 evidence: product **FAIL/freeze**, not CLOSED.
- **G1 manual west FAIL (122212 / enter 122244):** newest eye after post-drain kick landing. Same west corridor; VB all/fly med **78/78.5**, `focus_missing` med **1**, `miss_stuck` max **311**, StaleVL fly ~**77**, `gpu_kick` fly med **0**. `gpu_kick_post_drain_n` max **1** (single hit under stuck) — kick path lives but does **not** clear product FAIL. Enter settle still `live_blockers` (visibility_debt **80**, void_near ~**1417**).
- **G1 product autofly proxy:** `python tools/flight_sim_run.py --scenario product-174657` (yaw **180**, pin locus~(7,3), fog_pull_in off for run, idle15/fly38/stop20). Baseline cold `094214` / warm `094516`: early west VB **~115**. Post-RelightReplace F5 cold `095434` / warm `095703`: early VB **~92–109**. Route JSON: `tools/manual_flight_world164_product_174657.json`. Reports: `bin/suite_reports/g1_a10_relight/`.
- **G1 proxy_v2 adequacy (2026-09-14):** cold `112755` / visible `113011` fail-closed (`focus_missing=0`, `miss_stuck=0`, `fog_rd≈2`, VB fly med **20.5/13.5`). Root cause vs manual eye: **`hold_space` climb** (player_y **~76→300** vs manual **~50→58**); trees leave FOV; miss-class never forms.
- **G1 proxy_v3 (2026-09-14):** `hold_space=False`, `min-alt-above-sea=0`, `cruise-eye-y=56`, altitude + west-travel adequacy gates. Cold `125123` / warm `125331` both **adequacy PASS**: VB fly med **80.5/83.5**, `focus_missing` **1**, `miss_stuck` tail **158/184**, fog_rd **4**, Y **50→60**, focus `(7,3)→(2,3)`, `gpu_kick` fly med **1**. Post-drain max **1** (cold) / **0** (warm). Product G1 still **OPEN**.
- **G1-P3 FM-first + second Finish (2026-09-14):** under live focus miss, RemeshQ snapshot slice runs after FirstMesh; debt-forced kick may same-tick Finish even when HoleDrain backlog <12. Unit: `ShouldDeferRemeshSnapshotForFocusMiss` / `ShouldRunSecondGpuFinishPass`. Cold after patch `133817` still adequacy PASS but product OPEN (`miss_stuck` tail **170**, VB fly **94**, dominant `zero_fm_cap`). Next: why FirstMesh schedule cap/ok_fm stays starved under miss.
- **G1-P3 visual regress freeze (2026-09-14):** west manual `134914` / enter `134941` vs pre-P3 `122212` / enter `122244` (same corridor `(7,3)→(−3,3)`). Fly: VB **84** (was **78.5**), StaleVL **87** (was **77**), `miss_stuck` med/max **197/496** (was **85/263**), `dirty_remesh` **79** (was **54**), unlit max **39** (was **11**); unfinished **0** (was **2**, better). Proxy cold `133817` vs `125123`: VB **94** vs **80.5**, StaleVL **94.5** vs **80**. Root: `ShouldDeferRemeshSnapshotForFocusMiss` is an A10/A11 order-toggle (zoo policy) — `ok_fm` stays **0** while RemeshLit snapshot is delayed → black FullyDark/StaleVL chunks. **proxy_v3** remains adequacy harness only (altitude/west), not product CLOSED. Dual-lane schedule contract (fixed remesh-snap→FM order + lane quotas) replaces FM-first; no SoftDefer/floors/gate weaken.
- **G1 dual-lane schedule (2026-09-14):** removed miss-driven remesh-snapshot defer; `ComputeDualLaneSchedule` lane quotas in admission; Cache fixed remesh-snap→FM order; ad-hoc `first_mesh_cap=min(1)` removed; telem `schedule_lane_starve_reason`. Autofly gates: S1 cold StaleVL **83** / VB **84**; S3 cold StaleVL **84.5** / VB **84.5** / unlit_max **12**; S3 warm (r2) StaleVL **75** / VB **75** / unlit_max **16** (warm anchor `125331` unlit_max **19**). Reports: `dual_lane_s1_cold.json`, `dual_lane_s3_*.json`, `dual_lane_s3_gates.json`. Second Finish **kept**. Product G1 still **OPEN**.
- **G1 dual-lane west manual eye PASS (154921 / enter 154948):** same corridor `(7,3)→(−3,3)`, Y ~48–70. Operator: visually correct (no edit stress). Mid-corridor vs `122212` / `134914`: VB med **77** (78.5 / 84), StaleVL **75** (77 / 87), dirty_remesh **50** (54 / 79), ok_remesh med **2** (1 / 1), miss_stuck med/max **93/384** (85/263 / 197/496), cruise unlit max **13** (11 / 39), unfinished med **5** (vs ~2–4.5 / 0.5). **P3 black-chunk regress closed** vs `134914`. Wall med **~71 ms (~14 FPS)** — not a dual-lane regression (`122212` wall **~78 ms**; `134914` **~65 ms** was cheaper emerge with black lit). Wall split: stream **~31** + mesh_emerge **~28**. Enter still `live_blockers` (visibility_debt **80**, void_near **1457**). `ok_fm` med **0** remains (S5). Do **not** claim G1 product CLOSED vs 141350.
- **Freeze status:** product remains **OPEN**. Dual-lane visual SoT west = **154921** (closes `134914`); prior eye SoT **122212** still valid as pre-P3 baseline; P3 regress SoT = **134914**. Next: separate perf track (stream/emerge wall), not schedule order toggles.
- **G1 A10 RelightReplace Dirty sole-owner (`c3010053`):** secondary FullyDark Dirty pumps gated; MarkRelit→RemeshQ sole producer. Product still open — product gates vs 141350 still required for CLOSED.
