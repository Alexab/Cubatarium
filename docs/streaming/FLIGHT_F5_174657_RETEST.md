...
- **G1 manual west FAIL (100645 / enter 100713):** same no-teleport west corridor `(7,3)→(−3,3)` after RelightReplace Dirty sole-owner. VB all/fly med **51/76**, `focus_missing` med **1**, `miss_stuck` max **616**, `gpu_kick` fly med **0** (non-zero only sporadically), oracle StaleVL/FullyDark debt fly ~**74**. This is the new **E3 evidence**: product still **FAIL/freeze**, not CLOSED.
- **G1 product autofly proxy:** `python tools/flight_sim_run.py --scenario product-174657` (yaw **180**, pin locus~(7,3), fog_pull_in off for run, idle15/fly45/stop30). Baseline cold `094214` / warm `094516`: early west VB **~115**, full-run VB med heal ≤20 — not CLOSED. Post-RelightReplace F5 cold `095434` / warm `095703`: early VB **~92–109**. Route JSON: `tools/manual_flight_world164_product_174657.json`. Reports: `bin/suite_reports/g1_a10_relight/`.
- Proxy adequacy remains open after `100645`: west path and early VB align, but `focus_missing` / `miss_stuck` remain **0** in proxy runs, so `product-174657` is still insufficient to close G1 without a stronger proxy.
- **G1 proxy_v2 adequacy (2026-09-14):** defaults tightened to idle15 / fly38 / stop20 and report now writes `proxy_adequacy`. Cold `112755` and visible-cold `113011` both fail-closed with `focus_missing=0`, `miss_stuck=0`, `fog_rd_fly≈2`, VB fly med **20.5 / 13.5**. This hardens the proxy, but does **not** reproduce manual `100645` class.
- **Post-drain kick verification:** code landed (`gpu_kick_post_drain_n`, `gpu_kick_skip_no_queued_n`), unit covered, but proxy_v2 never reaches focus-miss class, so the new path is **not exercised** by autofly yet. No `N3` MarkRelit follow-up was justified from proxy evidence.
- **Freeze status:** product remains **OPEN**. In this run we can only freeze on existing manual `100645`; no new true west manual eye was produced here.
