...
- **G1 proxy_v3 (2026-09-14):** `hold_space=False`, `min-alt-above-sea=0`, `cruise-eye-y=56`, altitude + west-travel adequacy gates. Cold `125123` / warm `125331` both **adequacy PASS**: VB fly med **80.5/83.5**, `focus_missing` **1**, `miss_stuck` tail **158/184**, fog_rd **4**, Y **50→60**, focus `(7,3)→(2,3)`, `gpu_kick` fly med **1**. Post-drain max **1** (cold) / **0** (warm). Product G1 still **OPEN** (kick under miss does not clear stuck/VB vs 141350). Stuck-spawn without cruise-eye (`124719`) fail-closed via `focus_not_west`.
