...
- **G1 dual-lane schedule (2026-09-14):** removed miss-driven remesh-snapshot defer; `ComputeDualLaneSchedule` lane quotas in admission; Cache fixed remesh-snap→FM order; ad-hoc `first_mesh_cap=min(1)` removed; telem `schedule_lane_starve_reason`. Autofly gates: S1 cold StaleVL **83** / VB **84**; S3 cold StaleVL **84.5** / VB **84.5** / unlit_max **12**; S3 warm (r2) StaleVL **75** / VB **75** / unlit_max **16** (warm anchor `125331` unlit_max **19**). Reports: `dual_lane_s1_cold.json`, `dual_lane_s3_*.json`, `dual_lane_s3_gates.json`. Second Finish **kept**. Product G1 still **OPEN**. **West manual eye vs `122212` required** before claiming visual regress closed (operator): corridor `(7,3)→(−3,3)`, compare StaleVL/VB/unlit/black chunks to `122212` / avoid `134914` class.
- **Freeze status:** product remains **OPEN**. Visual SoT pre-P3 west = **122212**; P3 visual regress SoT = **134914**. Dual-lane autofly gates green; manual eye **pending**. Prior product FAIL freezes **122212** / **100645** still valid.
- **G1 A10 RelightReplace Dirty sole-owner (`c3010053`):** secondary FullyDark Dirty pumps gated; MarkRelit→RemeshQ sole producer. Product still open — **manual eye 174657-class required for CLOSED**.
