...
| TD-ITEM-003 | dig | Explicit block `dig.groups` on all packs | InferDigGroups heuristic until block agent fills JSON | block-agent |
...
| TD-ITEM-012 | dig | `ResolveDigParams` wired into `UWorld::ResolveBreakDurationSeconds` + wear on complete |
