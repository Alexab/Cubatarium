#!/usr/bin/env python3
"""Phase 5.6 scorecard: ring frontier + latch drain + holes/empty/ahead vs 192816.

Usage:
  python tools/AnalyzePhase56Scorecard.py \\
    --report bin/suite_reports/phase56_v0_flyheavy.json \\
    [--perf bin/logs/perf_....jsonl] \\
    [--info bin/logs/...INFO...] \\
    [--baseline-manual bin/logs/perf_20260906-192816_24828.jsonl] \\
    [--teleport false] \\
    [--tag v0]
"""
...
def print_control_checklist(perf: dict | None, info: dict | None, report: dict):
    print("=== Phase56 control checklist ===")
    ...
    if perf:
        ...
        print(
            ...
            f"(192816 miss_stuck=1469; not climb with kick=0)"
        )
        print(
...
def evaluate_fidelity(
    ...
    if teleport is True:
        fails.append("teleport_cruise=true forbidden for Phase56 gate")
...
def evaluate_product(info: dict | None, perf: dict | None) -> list[str]:
    """Product UX fails (Phase56: holes/empty/latch/miss_stuck)."""
    fails = []
    latch = None
    if perf:
    ...
    if perf:
        ...
        if holes is not None and float(holes) >= 0.50:
            fails.append(f"visual_holes_frac={holes:.3g}>=0.50 (192816 class)")
        empty_max = perf.get("empty_backlog_max")
        if empty_max is not None and float(empty_max) >= 15:
            fails.append(f"empty_backlog_max={empty_max:.0f}>=15 (192816 class)")
        clear_t = perf.get("latch_clear_t_ms")
...
def main():
    ...
    ap.add_argument(
        "--baseline-manual",
        default="bin/logs/perf_20260906-192816_24828.jsonl",
        help="manual perf JSONL for delta (default: 192816 SoT)",
    )
    ap.add_argument(
        "--teleport",
    ...
    print(f"=== Phase56 scorecard {args.tag} ===")
    ...
    baseline = dict(MANUAL_192816)
    baseline_label = "baked_192816"
    if args.baseline_manual:
        ...
        if bp.exists():
            ...
            print(f"baseline-manual missing: {bp} (using baked 192816)", file=sys.stderr)
