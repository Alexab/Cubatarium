...
ProceduralSettings ParseProceduralSettings(const nlohmann::json& root)
{
 ...
 const bool hasProcedural = root.contains("procedural") && root["procedural"].is_object();
 if (hasProcedural) {
  const nlohmann::json& p = root["procedural"];
  if (p.contains("generator") && p["generator"].is_string()) {
   settings.generator = ProceduralGeneratorFromString(p["generator"].get<std::string>());
  }
  if (p.contains("vertical") && p["vertical"].is_string()) {
   settings.vertical = VerticalModeFromString(p["vertical"].get<std::string>());
  }
 ...
 if (hasProcedural && root.contains("terrain") && root["terrain"].is_string()) {
  ...
  if (legacy != fromProc) {
   std::cerr << "WARN: procedural.generator (" << fromProc << ") overrides legacy terrain ("
             << legacy << ")" << std::endl;
  }
 }
