...
float SamplePerlin2D(float x, float z, const std::array<int, 512>& perm)
{
 ...
 const float x1 = Lerp(Grad2D(perm[aa], xf, zf), Grad2D(perm[ba], xf - 1.0f, zf), u);
 const float x2 = Lerp(Grad2D(perm[ab], xf, zf - 1.0f), Grad2D(perm[bb], xf - 1.0f, zf - 1.0f), u);
 return Lerp(x1, x2, v);
}

float SamplePerlin3D(float x, float y, float z, const std::array<int, 512>& perm)
{
 const int xi = static_cast<int>(std::floor(x)) & 255;
...
float FBM2D(float x, float z, uint32_t seed, int octaves, float persistence, float lacunarity)
{
 const auto perm = PermutationForSeed(seed);
 float value = 0.0f;
...
float FBM3D(float x, float y, float z, uint32_t seed, int octaves, float persistence, float lacunarity)
{
 const auto perm = PermutationForSeed(seed);
 float value = 0.0f;
 float amplitude = 1.0f;
 float frequency = 1.0f;
 for (int i = 0; i < octaves; ++i) {
  value += SamplePerlin3D(x * frequency, y * frequency, z * frequency, perm) * amplitude;
  amplitude *= persistence;
  frequency *= lacunarity;
 }
 return value;
