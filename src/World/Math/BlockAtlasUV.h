...
inline glm::vec3 BlockCenterFromFacePoint(const glm::vec3 &worldPos,
...
/// Atlas UV for a point on a cube face; matches InitCubeBuffers / UCubeGL
/// layout.
inline glm::vec2 AtlasUVFromWorldPos(int faceIndex, const glm::vec3 &worldPos)
{
  ...
  {
  case 0: // +Z NEAR
    return glm::vec2(glm::mix(u0, u1, local.x + 0.5f),
                     glm::mix(1.0f, 0.0f, local.y + 0.5f));
  case 1: // +X RIGHT
    return glm::vec2(glm::mix(u0, u1, 0.5f - local.z),
                     glm::mix(1.0f, 0.0f, local.y + 0.5f));
  case 2: // -Z FAR
    return glm::vec2(glm::mix(u0, u1, 0.5f - local.x),
                     glm::mix(1.0f, 0.0f, local.y + 0.5f));
  case 3: // -X LEFT
    return glm::vec2(glm::mix(u0, u1, local.z + 0.5f),
                     glm::mix(1.0f, 0.0f, local.y + 0.5f));
  case 4: // +Y TOP
    return glm::vec2(glm::mix(u0, u1, local.x + 0.5f),
                     glm::mix(0.0f, 1.0f, 0.5f - local.z));
  default: // -Y BOTTOM
    return glm::vec2(glm::mix(u0, u1, local.x + 0.5f),
                     glm::mix(0.0f, 1.0f, local.z + 0.5f));
  }
}
