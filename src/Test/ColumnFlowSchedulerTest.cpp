...
int main()
{
  ...
  s.Enqueue({1, 2}, ColumnWorkKind::FirstMesh, 99); // bump, logical size 1
  Expect(s.Size() == 1, "dedupe same column+kind (logical)");
  s.Enqueue({1, 2}, ColumnWorkKind::RelightThenMesh, 5);
  Expect(s.Size() == 2, "different kind not deduped");
  ColumnWorkItem a{}, b{};
  Expect(s.DrainOne(a), "drain first");
  Expect(a.priority == 99, "bumped FirstMesh priority drains first");
  ...
  Expect(s.Size() == 1, "RemeshSeam ticket bumped (logical size 1)");
  ...
  Expect(c.priority == 99, "RemeshSeam priority bumped");
