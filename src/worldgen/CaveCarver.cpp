...
bool ShouldCarve(int x, int y, int z, int surfaceY, uint32_t seed, const CaveParams& params)
{
 ...
 const float n = FBM3D(static_cast<float>(x) * params.scale,
     ...
     seed + 3000, params.octaves, params.persistence, params.lacunarity);
 const float n01 = (n + 1.0f) * 0.5f;
 return n01 > params.threshold;
}

void CarveColumnCaves(WorldGenContext& ctx, int x, int z, int surfaceY, uint32_t seed, const CaveParams& params)
{
