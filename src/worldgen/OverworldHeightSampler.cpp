...
HeightSampleParams ParamsForPreset(HeightPreset preset, int maxHeight)
{
 ...
 const float scale = maxHeight > 15 ? static_cast<float>(maxHeight) / 12.0f : 1.0f;
 switch (preset) {
 ...
 , params_(ParamsForPreset(preset, maxHeight))
{
 if (preset == HeightPreset::Mountains) {
  params_.stoneSurfaceAboveY = seaLevel + static_cast<int>(12.0f * (maxHeight > 15 ? maxHeight / 96.0f : 1.0f));
 }
}
