#ifndef SKELETAL_MODEL_SPACE_H
#define SKELETAL_MODEL_SPACE_H
...
inline glm::vec3 SkeletalUnitsToBlocks(const glm::vec3 &geoUnits)
{
  return geoUnits / kSkeletalUnitsPerBlock;
}

/// Minimum thickness for skeletal cubes with a zero axis (wings, legs, stinger).
inline float SkeletalThinAxisBlocks()
{
  return 1.f / kSkeletalUnitsPerBlock;
}

/// Which faces to emit when a cube has zero thickness on one axis.
inline bool SkeletalCubeFaceVisible(int faceIndex, const glm::vec3 &sizeBlocks)
{
  if (sizeBlocks.x <= 0.f)
  {
...
inline glm::vec3 SkeletalVisibleBoundsOffsetBlocks(const glm::vec3 &raw)
{
  return raw;
}

/// Entity forward −Z → Cubatarium +Z. Applied on the assembled skeleton at draw time.
inline glm::mat4 SkeletalEntityConventionMatrix()
{
...
inline glm::mat4 SkeletalPreviewRootMatrix(const CreatureSkeletalGeometry &geometry,
                                          float targetSpanBlocks)
{
  ...
  m = m * SkeletalEntityConventionMatrix();
...
inline glm::vec3 SkeletalBindPoseRotationDeg(const glm::vec3 &geoDeg)
{
  return glm::vec3(-geoDeg.x, -geoDeg.y, geoDeg.z);
}

/// Skeletal bone/cube rotation order: X, then Y, then Z (degrees).
inline glm::mat4 SkeletalEulerDegToMat(const glm::vec3 &eulerDeg)
{
  glm::mat4 m(1.f);
  if (eulerDeg.z != 0.f)
  {
