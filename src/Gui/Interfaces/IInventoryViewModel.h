...
public:
    ...
    virtual std::vector<InventoryGroupView> GetGroups(ContentKind tab, InventoryMode mode) const = 0;
    virtual std::vector<InventoryEntryView> GetEntries(ContentKind tab,
                                                       const std::string& groupId,
                                                       InventoryMode mode) const = 0;
    virtual bool CanAssignToHotbar(const InventoryEntryRef& entry,
                                   size_t barIndex,
                                   size_t slotIndex) const = 0;
    virtual bool AssignToHotbar(const InventoryEntryRef& entry,
                                size_t barIndex,
                                size_t slotIndex) = 0;
