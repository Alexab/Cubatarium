...
std::vector<int> ComputeColumnWidths(int availableW, int columns,
...
GridPlacementData BuildGridPlacementData(const GuiRect &clientArea,
                                         const GuiGridSpec &spec,
                                         const std::vector<GuiGridItem> &items)
{
  ...
  for (const auto &kv : pendingSpanHeights)
  {
    ...
    int current = 0;
    for (int r = row; r < row + rowSpan && r < static_cast<int>(d.rowH.size());
         ++r)
    {
      current += d.rowH[r];
    }
    ...
    {
      const int add =
          missing / rowSpan + ((r - row) < (missing % rowSpan) ? 1 : 0);
      d.rowH[r] += add;
    }
  }
  return d;
...
void UGuiLayout::AnchorChild(const GuiRect &clientArea, GuiAnchorKind kind,
                           ...
                           const std::vector<GuiGridItem> &items)
{
  ...
  for (const GuiGridItem &it : items)
  {
    ...
    const int row =
        std::clamp(it.row, 0, std::max(0, static_cast<int>(d.rowH.size()) - 1));
    const int colSpan = std::max(1, it.colSpan);
    const int rowSpan = std::max(1, it.rowSpan);
    const int colEnd = std::min(d.columns - 1, col + colSpan - 1);
    const int rowEnd =
        std::min(static_cast<int>(d.rowH.size()) - 1, row + rowSpan - 1);
    ...
    it.widget->SetBounds(
        {d.colX[col], rowY[row], std::max(0, w), std::max(0, h)});
  }
}
