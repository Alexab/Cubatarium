...
void UWindowManager::SetupCallbacks() 
{
  ...
  glfwSetFramebufferSizeCallback(Window, UInputManager::GLFWFramebufferSizeCallback);
  ...
  {
    auto* self = static_cast<UWindowManager*>(glfwGetWindowUserPointer(win));
...
void UWindowManager::Update() 
{
  ...
  if (Core && World && Application && Application->GetState() == AppState::InGame) 
  {
    const auto now = std::chrono::steady_clock::now();
    const double elapsed = std::chrono::duration<double>(now - LastAutosaveTime).count();
...
void UWindowManager::HandleMouseButtonEvent(MouseButton button, bool pressed, glm::vec2 pos) 
{
  const glm::ivec2 fbPos = CursorToFramebufferPixels(Window, pos.x, pos.y);
  ...
  if (Application && Application->RouteMouseButton(glfwButton, pressed, fbPos.x, fbPos.y)) 
  {
    return;
  }
  ...
  if (Application && Application->WantsCaptureMouse()) 
  {
    const bool allowPlace =
        button == MouseButton::Left && !pressed && Application->AllowsWorldMousePlacement();
    if (!allowPlace) 
    {
      return;
    }
...
void UWindowManager::SetTextRenderer(std::shared_ptr<UTextRenderer> text_renderer) 
{
...
glm::vec4 UWindowManager::GetSkyColor() const 
{
  return SkyColor;
}
...
bool UWindowManager::IsGradientSky() const 
{
  return UseGradientSky;
}

// GLFW callback functions
void UWindowManager::FramebufferSizeCallback(GLFWwindow* window, int width, int height) 
{
  UInputManager::GLFWFramebufferSizeCallback(window, width, height);
}

void UWindowManager::KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) 
{
  UInputManager::GLFWKeyCallback(window, key, scancode, action, mods);
}

void UWindowManager::MouseButtonCallback(GLFWwindow* window, int button, int action, int mods) 
{
  UInputManager::GLFWMouseButtonCallback(window, button, action, mods);
}

void UWindowManager::CursorPosCallback(GLFWwindow* window, double xpos, double ypos) 
{
  UInputManager::GLFWCursorPosCallback(window, xpos, ypos);
}

void UWindowManager::ScrollCallback(GLFWwindow* window, double xoffset, double yoffset) 
{
