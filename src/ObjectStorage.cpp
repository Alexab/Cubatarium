...
UObjectStorage::UObjectStorage(
    std::shared_ptr<UTextureCubeStorage> texture_cube)
...
void UObjectStorage::Generate()
{
  ...
  // AddPrototype(stone);
  // UObjectPrototype tree_birch("tree_birch", 4,
  // std::make_shared<USingleCube>(4)); AddPrototype(tree_birch);
}

void UObjectStorage::Load(const std::string &objects_path)
{
#ifdef CUBATARIUM_DEBUG
  std::cout << "UObjectStorage::Load: Loading from " << objects_path
            << std::endl;
#endif
...
#ifdef CUBATARIUM_DEBUG
    std::cout << "UObjectStorage::Load: Total loaded prototypes: "
...
const UObjectPrototype &
UObjectStorage::GetPrototype(const std::string &type_name) const
{
  auto type_id = GetObjectTypeId(type_name);
  if (type_id == 0)
  {
    std::cerr << "UObjectStorage::GetPrototype: unknown type '" << type_name
              << "'" << std::endl;
...
UObjectStorage::TakeObject(const std::string &type_name)
{
                              ...
                              std::vector<std::string> &cube_textures)
{
  ...
  try
  {
    ...
    if (name_value.empty() || id_value == 0 || cube_textures_value.empty() ||
        class_value.empty())
      return false;
